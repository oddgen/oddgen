...
class NewXtendGenerator implements OddgenGenerator2 {
	...
	override generate(Connection conn, Node node) {
		...
		val sourceDir = '''«outputDir»«File.separator»src«File.separator»main«File.separator»java«File.separator»«node.params.get(PACKAGE_NAME).replace(".",File.separator)»'''
		...
		result = '''
			...
			«writeToFile('''«sourceDir»«File.separator»«node.params.get(CLASS_NAME)».xtend''', generatorXtendTemplate.toString)»
